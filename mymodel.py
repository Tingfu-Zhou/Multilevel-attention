...
# this this Multilevel Attention (changed part)
def attention(q: torch.Tensor,
    ...
    num = 100 # num of loop of multi-level attention
    ...
    for i in range(num):
        tmp = torch.matmul(temp_q, temp_k.transpose(-2, -1)) / temp_d_k**0.5
        if mask is not None:
            tmp.masked_fill_(mask, -MY_INF)

        tmp = F.softmax(tmp, -1)
    tmp = torch.matmul(tmp, temp_v)
    temp_v = tmp
...
class MultiHeadAttention(nn.Module):
    ...
    def forward(self,
        ...
        attention_res = attention(q_, k_, v_, mask) #bug 11.27 18:27 step5
...
class EncoderLayer(nn.Module):
    ...
    def forward(self, x, src_mask: Optional[torch.Tensor] = None):
        tmp = self.self_attention(x, x, x, src_mask) #bug 11.27 15:27 step4
...
class DecoderLayer(nn.Module):
    ...
    def forward(self,
        ...
        tmp = self.dropout1(tmp)
        x = self.norm1(x + tmp)
        tmp = self.attention(x, encoder_kv, encoder_kv, src_dst_mask) #bug 11.27 18:27 step4
        tmp = self.dropout2(tmp)
        x = self.norm2(x + tmp)
        tmp = self.ffn(x)
...
class Encoder(nn.Module):
    ...
    def forward(self, x, src_mask: Optional[torch.Tensor] = None):
        ...
        for layer in self.layers:
            x = layer(x, src_mask) #bug 11.27 15:27 step3
        return x
...
class Decoder(nn.Module):
    ...
    def forward(self,
        ...
        x = self.dropout(x)
        for layer in self.layers:
            x = layer(x, encoder_kv, dst_mask, src_dst_mask) #bug 11.27 18:27 step3
        return x
...
class MATransformer(nn.Module):
    ...
    def generate_mask(self,
        ...
        if with_left_mask: # 1 means -inf in the position 
            mask = 1 - torch.tril(torch.ones(mask_shape))
        else:
            mask = torch.zeros(mask_shape)
        mask = mask.to(q_pad.device)
        for i in range(n): # q_pad: whcih row is useless, k_pad: which col is useless
            mask[i, :, q_pad[i], :] = 1
            mask[i, :, :, k_pad[i]] = 1
    ...
    def forward(self, x, y):
        ...
        res = self.decoder(y, encoder_kv, dst_mask, src_dst_mask) #bug 11.27 18:27 step2
